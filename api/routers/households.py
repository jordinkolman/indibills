...
async def get_households(*, session: Session = Depends(get_session), offset: int = 0, limit: int = Query(default=100, le=100)):
...
@router.post("/")
async def create_household(*, session: Session = Depends(get_session), household: HouseholdCreate):
...
async def get_household_by_id(*, session: Session = Depends(get_session), household_id: int):
    try:
        household = session.exec(select(Household).where(Household.id == household_id)).one()
...
async def update_household(*, session: Session = Depends(get_session), household_id: int, household: HouseholdUpdate):
...
async def delete_household(*, session: Session = Depends(get_session), household_id: int):
