...
@router.get("/user/{user_id}", response_model=list[ManualAccountPublic])
async def get_manual_accounts_by_user_id(
    *, session: Session = Depends(get_session), user_id: int
    ...
    return session.exec(
        select(ManualAccount).where(ManualAccount.user_id == user.id)
    ).all()


@router.get("/account/{account_id}", response_model=ManualAccountPublic)
async def get_manual_account_by_id(
    *, session: Session = Depends(get_session), account_id: int
):
    try:
        account = session.exec(
            select(ManualAccount).where(ManualAccount.id == account_id)
        ).one()
...
async def create_manual_account(
    *, session: Session = Depends(get_session), account: ManualAccountCreate
):
...
@router.patch("/account/{account_id}", response_model=ManualAccountPublic)
async def update_manual_account(
    *,
    session: Session = Depends(get_session),
    account_id: int,
    account: ManualAccountUpdate,
):
